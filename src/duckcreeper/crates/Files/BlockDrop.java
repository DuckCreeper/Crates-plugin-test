...
public class BlockDrop implements FileInterface {
	...
	public BlockDrop() {
		file = new File("plugins/Crates/Drops/block drops.yml");
	...
	public boolean exists() {
		if(new File("plugins/Crates/Drops/block drops.yml").exists()) return true;
		return false;
	}
	...
	public void create() {
		...
		fileconfig.set("Block that drop.EMERALD_ORE.chance", 0.2);
		keysThatDrop = new ArrayList<>();
		keysThatDrop.add("default");
		keysThatDrop.add("golden");
		fileconfig.set("Block that drop.EMERALD_ORE.keysThatDrop", keysThatDrop);
